...
async function renderCategoryDetails(req, res) {
    try {
...
async function categoryUnpublish(req,res){
    try {
        ...
        const category = await Category.findByIdAndUpdate(id, { isPublished: false }, { new: true });

        if (!category) {
            return res.status(404).json({ message: "Category not found" });
        }

        res.status(200).json({ message: "Category unpublished successfully", category });
    } catch (error) {
...
async function categoryPublish(req,res){
    try {
        const  id  = req.body.categoryId;
        const category = await Category.findByIdAndUpdate(id, { isPublished: true }, { new: true });
...
async function updateCategory(req,res){
    ...
    try {
        ...
        const newImageUrl = req.file ? `/uploads/categories/${req.file.filename}` : null;
...
module.exports={addCategories,renderCategoryDetails, categoryUnpublish,categoryPublish,updateCategory,categoryDelete}