...
async function renderCategoryOffersPage(req,res){
    try{
...
res.status(200).render(path.join("../", "views", "admin pages", "categoryOffer"), { category, offer,totalPages ,page,searchQuery });
...
async function addCategoryOffer(req, res) {
    ...
    try {
        ...
        await offer.save();
        return res.status(201).json({ message: "Category offer added successfully" });
...
async function editCategoryOffer(req, res) {
    try {
...
async function deleteCategoryOffer(req, res) {
    try {
        const offer = await categoryOffer.findOneAndDelete({ category: req.params.id });

        if (!offer) {
            return res.status(404).json({ message: "Error: Category offer not found" });
        }

        return res.status(200).json({ message: "Success: Category offer deleted successfully" });

    } catch (err) {
        console.error("Delete Error:", err);
        return res.status(500).json({ message: `Error: Internal server error - ${err.message}` });
    }
}


module.exports={renderCategoryOffersPage,addCategoryOffer,editCategoryOffer,deleteCategoryOffer}