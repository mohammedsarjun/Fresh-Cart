...
async function renderCouponPage(req,res){
  try {
...
coupon = await Coupon.find(filter).skip(skip).limit(limit);
    coupon = coupon.map(coupon => {
        return {
            ...coupon.toObject(), 
            couponStartDate: new Date(coupon.couponStartDate).toLocaleDateString("en-GB"),
            couponExpiryDate: new Date(coupon.couponExpiryDate).toLocaleDateString("en-GB")
        };
    });
    ...
    res.status(200).render(path.join("../", "views", "admin pages", "coupon"), { coupon,totalPages ,page,searchQuery});
...
async function addCoupon(req, res) {
  try {
    ...
    const existingCoupon = await Coupon.findOne({ couponCode: req.body.couponName });
    ...
    console.error(err);
    res.status(500).json({ message: "Server Error" }); // Proper error handling
  }
}
