...
async function renderProductOffersPage(req, res, next) {
  try {
    ...
    res
      .status(200)
      .render(path.join('../', 'views', 'admin pages', 'productOffers'), {
        product,
        offer,
        totalPages,
        page,
...
async function updateProductOffer(req, res, next) {
  try {
    ...
    if (req.body.variety != 'items') {
      offer = await productOffer.findOne({
        selectProduct: req.body.productOfferId,
        selectedVarietyMeasurement: req.body.varietyMeasurement,
      });
    } else {
      offer = await productOffer.findOne({
        selectProduct: req.body.productOfferId,
...
async function deleteProductOffer(req, res, next) {
  try {
    let deletedOffer = '';
    ...
    if (req.body.productVariety != 'items') {
      ...
      await product.save();
      deletedOffer = await productOffer.findOneAndDelete({
        selectProduct: req.params.id,
        selectedVarietyMeasurement: req.body.varietyMeasurement,
      });
    } else {
