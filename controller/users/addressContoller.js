...
async function userAddressRender(req,res){
    try {
        ...
        res.status(200).render(path.join("../", "views", "UserPages", "accountSettings","userAddress"),{userAddress})
    } catch (error) {
...
async function addUserAddress(req,res){
    try{
        ...
        const isDefaultAddress=await Address.findOne({userId:req.session.userId})
        if(!isDefaultAddress){
...
async function updateDefaultAddress(req,res){
    try{
        const address=await Address.findOne({_id:req.body.addressId})
        const defaultAddress=await Address.findOne({userId:req.session.userId,isDefault:true})
        if(defaultAddress){
