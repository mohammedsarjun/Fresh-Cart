 ...
 await userCart.save()
    ...
    for(let i=0;i<userCart.products.length;i++){
      ...
      if(productDetail.variety!="items"){
        const productStock=productDetail.varietyDetails.filter((varietyDetail)=>varietyDetail.varietyMeasurement==userCart.products[i].selectedVariety.value)[0].varietyStock
    ...
    for(let i=0;i<userCart.products.length;i++){
      ...
      if(productDetail.variety!="items"){
        ...
        const varietyDetail=productDetail?.varietyDetails.filter((varietyDetail)=>varietyDetail.varietyMeasurement == selectedVariety)[0]
        
        const price = Object.values(productDetail.productPrice).find((price) => price != null);
       
        const productPrice = varietyDetail.varietyMeasurement * price * userCart.products[i].quantity;
        
        const productDiscount=productDetail.varietyDetails.filter((varietyDetail)=>varietyDetail.varietyMeasurement==userCart.products[i].selectedVariety.value)[0].varietyDiscount
        let totalPrice = productPrice - (productPrice * productDiscount) / 100;
      ...
      cart.products.map(async (product) => {
        ...
        if (product.selectedVariety.type != "items") {
          ...
          discountPrice =
            ...
            isStock=productDetail.varietyDetails[0].itemStock>0?true:false
          ...
          if (req.body.variety != "items") {
            if (
              ...
              cart?.products[i]?.selectedVariety?.value ==
                ...
                let pricePerVariety=Object.values(productDetail.productPrice).filter((productPrice)=>productPrice!=null)[0]
                let productDiscount=productDetail.varietyDetails.filter((varietyDetail)=>varietyDetail.varietyMeasurement==cart.products[i].selectedVariety.value)[0].varietyDiscount
                cart.subtotal =cart.subtotal - (pricePerVariety * cart.products[i].selectedVariety.value) * (1 - productDiscount / 100) *cart.products[i].quantity
                cart.grandTotal=cart.subtotal+cart.shippingCost
            ...
            if (cart.products[i].productId == req.body.productId) {
                const productDetail=await Product.findOne({_id:cart.products[i].productId})
                cart.subtotal =cart.subtotal - productDetail.varietyDetails[0].varietyPrice * (1 - productDetail.varietyDetails[0].varietyDiscount / 100) *cart.products[i].quantity
                cart.grandTotal=cart.subtotal+cart.shippingCost
              cart.products.splice(i, 1);
              break;
            }
...
//----------------------------------------------------wishlist-------------------------------------------------//
async function wishlistPageRender(req, res) {
  try {
