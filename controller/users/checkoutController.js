...
async function checkoutPageRender(req,res){
  ...
  let cart = JSON.stringify(
    ...
    if(cart?.coupon?.discount){
      cart.grandTotal=cart.grandTotal-(cart.grandTotal*cart.coupon.discount)/100
    }
...
address.sort((a, b) => b.isDefault - a.isDefault);
 ...
 res
      .status(200)
      .render(
        path.join("../", "views", "UserPages", "orderSide", "checkout"),
        {address,cart}
      );
}
...
async function verifyCoupon(req, res) {
  try {
    ...
    if( discountAmount>coupon.maximumDiscount){
      console.log(cart.grandTotal - (cart.grandTotal * coupon.discountPercentage / 100),coupon.maximumDiscount)
      ...
      cart.grandTotal = cart.grandTotal - (cart.grandTotal * coupon.discountPercentage / 100);
    }
