...
async function productEditAction(
  productId,
  productImg1,
  productImg2,
  productImg3,
  ...
  if (variety != 'items') {
    ...
    window.varietyDetails
      .slice()
      .reverse()
      .forEach((varietyDetail) => {
        addVariety(
          variety,
          varietyDetail.varietyMeasurement,
          varietyDetail.varietyDiscount,
          varietyDetail.varietyStock
        );
      });
  } else {
    document.getElementById(`${variety}EditCheckbox`).checked = true;
...
document
  ...
  .addEventListener('submit', async function (event) {
    ...
    if (variety && variety !== 'items') {
      ...
      if (firstVariety) {
        formData.append(
          `pricePer${firstVariety}`,
          $('#editVarietyPrice').val()
        ); // Add dynamically
      }
    } else {
